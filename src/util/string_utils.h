    ...
    namespace str {
        ...
        inline string_view_t skipWhitespaceView(string_view_t string) {
            ...
            while (start < string.size() &&
                   std::isspace(static_cast<unsigned char>(string[start]))) {
                ++start;
            }
        ...
        /// @note - If `pattern` is empty, it will insert `replacement` before every character and
        /// at the end.
        /// @note - If `text_in_out` is empty, it will set `text_in_out` to `replacement` and
        /// return 1.
        /// @note - If `pattern` is not found in `text_in_out`, it will return 0 and leave
        /// `text_in_out` unchanged.
        ...
        /// Helper for number-to-string conversion, safe and lightweight to avoid using
        /// std::to_string
        /// @brief Convert a number to a string and store it in the provided buffer.
        /// @tparam T The type of the number (int, unsigned int, long, unsigned long, float,
        /// double).
        ...
        /// @brief Given a string, this function splits it into tokens based on a delimiter
        /// character.
