...
/// Host backend for `yield()`. `std::this_thread::yield()` hints the
/// OS scheduler to run another runnable thread; on a single-threaded
/// host test it is effectively a no-op, which is fine — there is no
/// watchdog to feed.
inline void yield()
{
