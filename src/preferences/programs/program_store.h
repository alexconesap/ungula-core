...
namespace ungula {
    ...
    class ProgramStore {
        public:
            explicit ProgramStore(IPreferences& prefs) : prefs_(prefs) {
            ...
            void init(const char* defaultName, ProgramT (*createDefault)(const char*)) {
                ...
                if (activeIndex_ < 0 || activeIndex_ >= MaxPrograms ||
                    !programs_[activeIndex_].valid) {
        ...
        private:
            ...
            void loadAllFromNvs() {
                ...
                for (int idx = 0; idx < MaxPrograms; idx++) {
                    ...
                    ProgramBlob blob;
                    size_t read =
                            prefs_.getBytes(key, reinterpret_cast<uint8_t*>(&blob), sizeof(blob));
                    if (read != sizeof(blob)) {
                        programs_[idx].valid = false;
                        continue;
                    }
