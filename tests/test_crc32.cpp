...
TEST(CRC32, EmptyDataReturnsZero) {
    uint8_t data[1] = {};
    uint32_t crc = ungula::crc32(data, 0);
...
TEST(CRC32, KnownStringValue) {
    ...
    uint32_t crc = ungula::crc32(reinterpret_cast<const uint8_t*>(str), 9);
    EXPECT_EQ(crc, 0xCBF43926u);
}

TEST(CRC32, SingleByte) {
    uint8_t data[1] = {0x00};
    uint32_t crc = ungula::crc32(data, 1);
    EXPECT_NE(crc, 0u);
}
...
TEST(CRC32, DifferentDataProducesDifferentCrc) {
    ...
    EXPECT_NE(ungula::crc32(data1, 4), ungula::crc32(data2, 4));
}

TEST(CRC32, SameDataProducesSameCrc) {
    uint8_t data[4] = {0xDE, 0xAD, 0xBE, 0xEF};
    EXPECT_EQ(ungula::crc32(data, 4), ungula::crc32(data, 4));
}

TEST(CRC32, ByteByByteMatchesBuffer) {
    const char* str = "Hello";
    uint32_t bufferCrc = ungula::crc32(reinterpret_cast<const uint8_t*>(str), 5);

    // Compute byte by byte
    uint32_t byteCrc = 0xFFFFFFFFu;
    for (int i = 0; i < 5; i++) {
        byteCrc = ungula::crc32_byte(byteCrc, static_cast<uint8_t>(str[i]));
    }
    byteCrc ^= 0xFFFFFFFFu;
